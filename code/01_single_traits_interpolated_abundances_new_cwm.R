...
## join new abundances
fish_ab_int <- full_join(mediansx, fish_ab_sub, by = "site_year", relationship = "many-to-many")
...
save(fish_ab_rel_int, file="sync/01_fish_abundances.RData")
...
# length(unique(fish_ab_rel_int$SiteID)) # 767
head(fish_ab_rel_int)
...
## Alain function
library(ecocom)
# install.packages("ecocom") ## not available for r version
...
## community mean and variance calculate
comMean <- abun_traits %>%
