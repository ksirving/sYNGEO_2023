  ...
  for (region in 1:length(regionsID)) {
    ...
    for (ax in 1: length(Ntraits)) {
      ...
      diversityBayes <- sapply(seq_len(nrow(cc)), function(k) {
        ...
        bayestestR::overlap(div_mat2$site1,div_mat2$site2, method_density = "KernSmooth")[[1]] ## overalp index
      ...
      # ## distance scaled
      # distanceScaled <- sapply(seq_len(nrow(cc)), function(k) {
      #   i <- cc[k,1]
      #   j <- cc[k,2]
      #   
      #   dist_mat <- matrix(
      #     c(scale(trait_CWM[, i]),scale(trait_CWM[,j])),
      #     nrow = 10,
      #     byrow = F,
      #     dimnames = list(years,
      #                     c("site1", "site2")
      ...
      # 
      ## combine all
      synchrony <- cbind(cc, synchrony, absdistance, distance, distanceAbs,  diversity2,  diversity3, diversityBayes)
      # synchrony
