...
## overlap v temp
tempDis <- sjPlot::plot_model(mem_mixed0, type="pred", terms= c("ZDistance", "ZTempCor [-1.86, 0.3, 0.92]"),
                              axis.title = c( "Trait Distance (Z Score)","Thermal Trait Synchrony"), 
                              legend.title = "Environmental synchrony")
tempDis


DisTemp <- sjPlot::plot_model(mem_mixed0, type="pred", terms= c("ZTempCor", "ZDistance [-1.094, -0.318, 2.004]"),
...
## overlap v temp
tempDiv <- sjPlot::plot_model(mem_mixed1, type="pred", terms= c("ZOverlap", "ZTempCor [-1.86, 0.3, 0.92]"),
                              axis.title = c( "Trait Overlap (Z Score)","Thermal Trait Synchrony"), 
                              legend.title = "Environmental synchrony")
tempDiv


DivTemp <- sjPlot::plot_model(mem_mixed1, type="pred", terms= c("ZTempCor", " ZOverlap [-1.436, 0.053, 1.499]"),
                              axis.title = c("Environmental synchrony (Z Score)" ,"Thermal Trait Synchrony"), 
...
## temp
meanTemp <- allsyncxWithin %>% group_by(Region) %>% summarise(mean.vari.temp = mean(annual_avgC),
                                                            sd.vari.temp = sd(annual_avgC))
meanTemp 

BackTemp <- allsyncxWithin %>% full_join(meanTemp, by = "Region") %>% group_by(Region) %>% summarise(TempBack = (ZTempCor * sd.vari.temp) + mean.vari.temp)
BackTemp

class(transVals)
transVals
trBack <- function(x) (x*transVals)
trBackInv <- function(x) (x/transVals)
#attach(transVals)
#transVals <- as.data.frame(matrix(allsyncxWithin$annual_avg/ allsyncxWithin$ZTempCor[1:100]))
transVals <- allsyncxWithin$annual_avgC/ allsyncxWithin$ZTempCor

allsyncxWithin$ZTempCor[1:100]*transVals
allsyncxWithin$annual_avgC[1:100]/transVals
allsyncxWithin$annual_avgC[1:100]
allsyncxWithin$ZTempCor[1:100]

trBack(allsyncxWithin$ZTempCor)[1:100]
trBackInv(allsyncxWithin$annual_avgC)[1:100]

allsyncxWithin$annual_avgC

mean.vari.temp <- mean(allsyncxWithin$annual_avgC)
sd.vari.temp <- sd(allsyncxWithin$annual_avgC)
var.back.temp <- (allsyncxWithin$ZTempCor * sd.vari.temp) + mean.vari.temp
sd.vari.temp
...
DisTempx <- sjPlot::plot_model(mem_mixed0, type="pred", terms= c("ZTempCor", "ZDistance [-1.094, -0.318, 2.004]"),
  ...
  scale_color_manual(values=c("#56B4E9", "#E69F00",  "#999999"), labels = c("5th", "50th", "95th")) +
  #scale_color_manual(values= wes_palette("Zissou1", n = 3), labels = c("5th", "50th", "95th")) +
  scale_fill_manual(values=c("#56B4E9", "#E69F00",  "#999999")) +
  scale_linetype_manual(values = c("solid", "dashed", "dotted"), labels = c("5th", "50th", "95th")) +
  scale_x_continuous(trans = trans_new("Correlated Temp", 
                                       transform=trBack,
                                       inverse=trBackInv)) +
...
DivTemp <- sjPlot::plot_model(mem_mixed1, type="pred", terms= c("ZTempCor", " ZOverlap [-1.436, 0.053, 1.499]"),
  ...
  scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"), labels = c("5th", "50th", "95th")) +
  #scale_color_manual(values= wes_palette("Zissou1", n = 3), labels = c("5th", "50th", "95th")) +
...
df <- ggpredict(mem_mixed1, terms= c("ZTempCor", " ZOverlap [-1.436, 0.053, 1.499]"))
df$x2 <- (df$x * sd.vari.temp) + mean.vari.temp
df

ggplot(df, aes(x2, predicted)) + 
  geom_line(aes(linetype=group, color=group)) +
  geom_ribbon(aes(ymin=conf.low, ymax=conf.high, fill=group), alpha=0.15) +
  scale_color_manual(values=c("#999999", "#E69F00", "#56B4E9"), labels = c("5th", "50th", "95th")) +
  #scale_color_manual(values= wes_palette("Zissou1", n = 3), labels = c("5th", "50th", "95th")) +
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9"), labels = c("5th", "50th", "95th")) +
  scale_linetype_manual(values = c("solid", "dashed", "dotted"), labels = c("5th", "50th", "95th")) +
  scale_x_continuous()
...
## save out
file.name1 <- paste0(out.dir, "08_biotic_interactions_dist_overlap_corTemp.jpg")
